...
difference(){
	gregs_x_carriage();
	translate([0,0,-2.5]) cube([70,70,5], center=true);
}
//y_axis_holder ();
...
module gregs_x_carriage(with_fanmount=true) 
{
	difference()
	{
		union ()
		{
			...
			{
			barbell (
				[36-lm8uu_holder_width/2,holder_separation/2+16],
				[36-lm8uu_holder_width/2,-holder_separation/2-16],
				10,10,56,56);
		

			barbell (
				[36-lm8uu_holder_width/2,holder_separation/2+18],
				[-25-13.5+belt_clamp_hole_separation/2,1*(lm8uu_holder_length/2+belt_clamp_width/2)],
				8,belt_clamp_width/2,130,230);
...
//			belt_support_offset=25+17.5;
	...
	difference ()
	{
		union ()
		{
			for(i=[-1,1])
			{
				translate([50/2, i*(lm8uu_holder_length+holder_separation)/2,lm8uu_support_thickness])
				render()
				standart_bushing(center=true);
			}
			translate([-50/2,0,lm8uu_support_thickness])
			render()
			standart_bushing();
		}
	}		
