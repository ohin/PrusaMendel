...
difference(){
	union(){
		...
		#translate([outer_diameter-3, outer_diameter, 0]) cube([6, 4+distance_from_rod+1, 10]);
		translate([outer_diameter+3-40, outer_diameter+distance_from_rod+4, 0]) cube([40, 6, 10]);
	...
	translate([-25,22,10/2-m3_diameter/2]) cube ([ 20,8,m3_diameter]);
	//Securing hole
